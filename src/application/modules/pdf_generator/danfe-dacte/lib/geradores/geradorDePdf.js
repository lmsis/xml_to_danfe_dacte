...
module.exports = function (danfe, args, callback) {
    ...
    var emitente = danfe.getEmitente(),
        ...
        deveExibirQuadroDeCalculoDoIssqn =
            ...
            impostos._baseDeCalculoDoIcms ||
            impostos._valorTotalDoIssqn;
    ...
    function desenharPagina() {
        ...
        linhaVertical(269, 309, 147);
        linhaVertical(269, 309, 221);
        linhaVertical(269, 309, 280);
        linhaVertical(269, 309, 343);
        linhaVertical(269, 309, 393);
        // linhaVertical(269, 309, 309);
        ...
        linhaVertical(320, 340, 340);
        linhaVertical(320, 340, 391);
        linhaVertical(320, 340, 465);
        linhaVertical(340, 360, 434);
        linhaVertical(320, 340, 486);
        linhaVertical(340, 360, 456.65);
        ...
        if (duplicatas.length) {
            adicional_Itens_y -= 1;
            linhasDeDuplicatas = Math.ceil(
                duplicatas.length / duplicatasPorLinha
            );
            ...
            pdf.font("codigoDeBarras")
                ...
                .text(
                    barcode.code128.encode(
                        formularioDeSeguranca.getDadosDaNfe()
                    ),
                    341.5,
                    131.5,
                    {
        ...
        var informacoesComplementares = [danfe.getInformacoesComplementares()];
        ...
        if (danfe.getChaveDeAcesso()) {
            pdf.font("codigoDeBarras")
                ...
                .text(
                    ...
                    args.ajusteX + margemEsquerda + 339,
                    args.ajusteY + margemTopo + 65,
                    {
                        align: "center",
                        width: 249,
                    }
        ...
        campo(danfe.getChaveDeAcesso(), 341.5, 114, 244);
        ...
        if (protocolo.getFormatacao()) {
            ...
            campo(
                formularioDeSeguranca.getDadosDaNfeFormatado(),
                341.5,
                158.4,
                244
            );
        ...
        campo(
            danfe.getInscricaoEstadualDoSubstitutoTributario(),
            197,
            178.4,
            192.5
        );
        ...
        campo(
            ...
            272,
            "left",
            args.tamanhoDaFonteDoCampo - 0.5
        );
        ...
        campo(impostos._baseDeCalculoDoIcms, 1.5, 278, 84, "right");

        titulo("VALOR DO ICMS", 89, 270, 84);
        campo(impostos._valorDoIcms, 89, 278, 54, "right");

        titulo("BASE DE CÁLC. ICMS S.T.", 149, 270, 84);
        campo(impostos._baseDeCalculoDoIcmsSt, 149, 278, 70, "right");

        titulo("V. DO ICMS SUBST.", 223, 270, 84);
        campo(impostos._ValorDoIcmsSt, 223, 278, 54, "right");

        titulo("VALOR IMP. IMPORT", 281, 270, 84);
        campo(impostos._valorDoImpostoDeImportacao, 281, 278, 54, "right");

        titulo("VALOR DO PIS", 441, 270, 47);
        campo(impostos._valorDoPis, 441, 278, 47, "right");

        //// aqui htardivo
        titulo("V. ICMS UF REM", 345, 270, 67);
        campo(impostos._valorICMSufRemet, 345, 278, 42, "right");

        titulo("V. ICMS UF DEST", 345, 290, 67);
        campo(impostos._valorICMSufDest, 345, 298, 42, "right");

        titulo("VALOR FCP", 395, 270, 67);
        campo(impostos._valorFCP, 395, 278, 42, "right");

        titulo("V. TOT. TRIB", 395, 290, 67);
        campo(impostos._valorTotTrib, 395, 298, 42, "right");
        ...
        campo(danfe.getValorDoSeguroFormatado(), 89, 298, 54, "right");

        titulo("DESCONTO", 149, 290, 84);
        campo(danfe.getDescontoFormatado(), 149, 298, 70, "right");

        titulo("OUTRAS DESPESAS", 223, 290, 84);
        campo(danfe.getOutrasDespesasFormatado(), 223, 298, 54, "right");

        titulo("VALOR TOTAL DO IPI", 281, 290, 84);
        campo(impostos._valorTotalDoIpi, 281, 298, 54, "right");
        ...
        titulo(
            ...
            47,
            null,
            args.tamanhoDaFonteDoTitulo - 1
        );
        campo(impostos._valorDaCofins, 440.5, 298, 47, "right");
        ...
        campo(transportador.getNome(), 1.5, 329, 280, "left");

        titulo("FRETE POR CONTA", 260, 321, 85);
        campo(danfe.getModalidadeDoFreteFormatada(), 260, 329, 85);

        titulo("CÓDIGO ANTT", 345, 321, 84);
        campo(transportador.getCodigoAntt(), 345, 329, 84);

        titulo("PLACA DO VEÍCULO", 397.5, 321, 84);
        campo(transportador.getPlacaDoVeiculoFormatada(), 397.5, 329, 84);

        titulo("UF", 470, 321, 19.5);
        campo(transportador.getUfDaPlacaDoVeiculo(), 470, 329, 19.5);

        titulo("CNPJ / CPF", 490, 321, 126.5);
        campo(transportador.getRegistroNacionalFormatado(), 482, 329, 126.5);
        ...
        if (fatura) {
            ...
            if (fatura.getNumero()) {
                dadosDaFatura.push(
                    "VALOR ORIGINAL: " + fatura.getValorOriginalFormatado()
                );
            }

            if (fatura.getNumero()) {
                dadosDaFatura.push(
                    "VALOR DO DESCONTO: " + fatura.getValorDoDescontoFormatado()
                );
            }

            if (fatura.getNumero()) {
                dadosDaFatura.push(
                    "VALOR LÍQUIDO: " + fatura.getValorLiquidoFormatado()
                );
            }

            if (fatura.getFormaDePagamento()) {
                dadosDaFatura.push(
                    "PAGAMENTO: " + fatura.getFormaDePagamento()
                );
            }
            ...
            if (dadosDaFatura) {
                normal(dadosDaFatura, 1.5, 390, larguraDoFormulario, "left", 6);
            }

            // linhaVertical(389, alturaDaSecaoDosItens - 1, larguraDoFormulario);
            // linhaVertical(389, alturaDaSecaoDosItens - 1, 0);
            // linhaHorizontal(0, 0, 397);
            // linhaHorizontal(0, 0, alturaDaSecaoDosItens - 1);
        }

        alturaDosTitulosDosItens = alturaDaSecaoDosItens + adicional_Itens_y;
        ...
        titulo(
            ...
            alturaDosTitulosDosItens,
            50.5,
            args.alinhamentoDoTituloDaTabela
        );
        titulo(
        ...
        if (deveExibirQuadroDeCalculoDoIssqn) {
            ...
            campo(
                danfe.getValorTotalDosServicosFormatado(),
                137.5,
                771,
                132.5,
                "right"
            ...
            campo(impostos._baseDeCalculoDoIssq, 273.5, 771, 132.5, "right");

            titulo("VALOR TOTAL DO ISSQN", 409.5, 763, 175.5);
            campo(impostos._valorTotalDoIssqn, 409.5, 771, 175.5, "right");
        }
    ...
    itens.forEach(function (item) {
        function renderizarLinha(pdf) {
            ...
            normal(
                item._aliquotaDoIcms,
                536,
                yDoItemAtual + 0.65,
                19,
                "center",
                args.tamanhoDaFonteDosItens - 1,
                pdf
            );
            maiorY = Math.max(maiorY, pdf.y);
